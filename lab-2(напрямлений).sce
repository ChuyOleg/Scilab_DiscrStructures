...
function buildLines(Matrix, n, ip)
  ...
  for k = 1:n
    if (degrees(k,2) - degrees(k, 3) == 0)
      isolatedVertex(count1, 1) = k;
      count1 = count1 + 1;
    end
    if (degrees(k,2) - degrees(k, 3) == 1)
      hangingVertex(count2, 1) = k;
      count2 = count2 + 1;
    end
  end
  disp(degrees, 'Напівстепені вершин напрямленого графа');
  disp(hangingVertex, 'Висячі вершини напрямленого графа');
  disp(isolatedVertex, 'Ізольовані вершини напрямленого графа');
endfunction
